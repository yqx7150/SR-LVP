...
def initialize_weights(net_l, scale=1):   #初始化网络参数权重
    if not isinstance(net_l, list):    #判断类型是不是我们指定的类型
        net_l = [net_l]
    for net in net_l:
        for m in net.modules():
            if isinstance(m, nn.Conv2d):
                init.kaiming_normal_(m.weight, a=0, mode='fan_in')
                m.weight.data *= scale  # for residual block     对于残差快
...
def initialize_weights_xavier(net_l, scale=1):   #初始化
...
class InvBlock(nn.Module):
    def __init__(self, subnet_constructor, channel_num, channel_split_num, clamp=0.8):
        ...
        self.split_len2 = channel_num - channel_split_num # 2                     #分割成一个一份和一个两份
        ...
        in_channels = 24                           #修改网络通道数？？？？怎么计算，根据输入大小深度进行修改？？？？
    ...
    def forward(self, x, rev=False):
        if not rev:
            ...
            x1, x2 = (x.narrow(1, 0, self.split_len1), x.narrow(1, self.split_len1, self.split_len2))       #（1，0，1）  （1，1，2）
...
class InvISPNet(nn.Module):
    def __init__(self, channel_in=24, channel_out=24, subnet_constructor=subnet('DBNet'), block_num=8):
        ...
        # channel_split_num = 1
        # channel_split_num = 4
        channel_split_num = 8    #分离的通道数！！ 要修改,一份占多少，1：2,需要修改
