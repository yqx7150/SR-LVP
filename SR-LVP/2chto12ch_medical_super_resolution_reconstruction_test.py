...
def main(args):
    ...
    if os.path.isfile(args.ckpt):
        net.load_state_dict(torch.load(args.ckpt), strict=False)  #加载网络参数
    ...
    mri_dataset = mriDataset12and4_real_imag_cross(root1='/home/lqg/文档/lxh/data_brain/test_data/test_12ch', root2='/home/lqg/文档/lxh/data_brain/test_data/test_2ch_SCC', root='/home/lqg/文档/lxh/data_brain/test_data/test_12ch')
    ...
    for i_batch, sample_batched in enumerate(tqdm(dataloader)):
        ...
        with torch.no_grad():  #测试不想需要求梯度优化网络参数
        ...
        pred_12_real_imag = reconstruct_12_real_imag.detach().permute(0,2,3,1).squeeze()    #压缩？？？？？？
        ...
        pred_12_real_imag = pred_12_real_imag.cpu().numpy()                                #转成numpy
        ...
        ori_4coil_complex = np.zeros([256, 256, 2], dtype=np.complex64)   #complex64实部虚部都是32位浮点                   #输入
        ...
        ori_complex = np.zeros([256, 256, 12], dtype=np.complex64)                                                             #标签
        for i in range(0, 24, 2):
            ori_complex[:, :, int(i/2)] = target_channel24_mri_real_imag[:, :, i] + 1j*target_channel24_mri_real_imag[:, :, i+1]
        ori_complex_sos = np.sqrt(np.sum(np.abs((ori_complex)**2), axis=2))   #在通道方向上求和
        ...
        pred_12_complex = np.zeros([256, 256, 12], dtype=np.complex64)                                                  #输出
        for i in range(0, 24, 2):
        ...
        plt.subplot(1 ,3, 1)
        plt.imshow(255*abs(ori_4coil_complex_sos) ,cmap='gray')      #输入，显示灰度图
        plt.title("ori_4_sos")
        plt.subplot(1 ,3, 2)
        plt.imshow(255*abs(ori_complex_sos) ,cmap='gray')                #标签
        plt.title("ori_12_sos")
        plt.subplot(1 ,3, 3)
        plt.imshow(255*abs(pred_12_complex_sos) ,cmap='gray')                       #输出
